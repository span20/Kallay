		...
		<table>
			...
			{foreach from=$page_data item=data}
				<tr class="{cycle values="row1,row2"}">
					...
					<td class="last">
                        <a href="admin.php?p={$self}&amp;act={$this_page}&amp;sub_act=fields_lst&amp;nid={$data.id}">
							<img src="{$theme_dir}/images/admin/gallery.gif" border="0" alt="{$lang_gallery.strAdminGalleryPics}" />
						</a>
						<a class="action mod" href="admin.php?p={$self}&amp;act={$this_page}&amp;sub_act=mod&amp;id={$data.id}" title="{$locale.admin_contents.news_tpl_list_modify}"></a>
                        <a class="action del" href="admin.php?p={$self}&amp;act={$this_page}&amp;sub_act=del&amp;id={$data.id}" title="{$locale.admin_contents.news_tpl_list_delete}"></a>
					</td>
				</tr>
			{foreachelse}
				<tr>
