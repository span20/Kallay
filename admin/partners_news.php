...
if (isset($_REQUEST['id']) && $_REQUEST['id'] != 0) {
    ...
    $acttpl = 'error';
    $tpl->assign('errormsg', $locale->get('news_error_none_selected'));
    return;
}
...
if ($sub_act == 'del') {
    $query = "
        DELETE FROM iShark_Partners_News 
		WHERE news_id = $id
    ";
    $mdb2->exec($query);
...
if ($sub_act == 'add' || $sub_act == 'mod') {
    ...
    $titles = array('add' => $locale->get('news_act_add'), 'mod' => $locale->get('news_act_mod'));
    ...
    $form->addElement('header', 'news_header', $titles[$sub_act]);
    $form->addElement('hidden', 'act',         $page);
    $form->addElement('hidden', 'sub_act',     $sub_act);
    $form->addElement('hidden', 'id',          $id);

    //hir cime
    $form->addElement('text', 'title', $locale->get('news_field_title'), 'maxlength="255"');
    ...
    $form->addGroup(
       ...
       'date_start', $locale->get('news_field_date_start'), null, false
    );
    $form->addGroup(
        array(
        ...
        'date_end', $locale->get('news_field_date_end'), null, false
    );

    //a hir
    $form->addElement('textarea', 'description', $locale->get('news_field_description'), 'id="description"');

    $form->applyFilter('__ALL__', 'trim');

    $form->addRule('title',       $locale->get('news_error_required_title'),       'required');
    $form->addRule('description', $locale->get('news_error_required_description'), 'required');
    ...
    if ($form->validate()) {
        ...
        if ($sub_act == 'mod') {
			...
			$news_id = $mdb2->extended->getBeforeID('iShark_Partners_News', 'news_id', TRUE, TRUE);
            $query = "
                INSERT INTO iShark_Partners_News 
				(news_id, title, timer_start, timer_end, description) 
				VALUES 
				($news_id, '$title', '$timer_start', '$timer_end', '$desc')
            ";
        }
        $mdb2->exec($query);
...
if ($sub_act == 'lst') {
    ...
    $table_headers = array(
        ...
        'timer_start' => $locale->get('news_field_date_start'),
        'timer_end'   => $locale->get('news_field_date_end'),
        '__act__'     => $locale->get('news_actions'),
    );

    // dynamic listhez szukseges nyelvi mezok
    $lang_dynamic = array(
        'strAdminEmpty'   => $locale->get('news_no_new'),
        'strAdminConfirm' => $locale->get('news_confirm_new')
    );

    // dynamic listhez szukseges mezomuveletek
    $actions_dynamic = array(
        'mod' => $locale->get('news_act_mod'),
        'del' => $locale->get('news_act_del'),
    );

    // Adatbazis lekerdezes
    $query = "  
        SELECT news_id as id, title, timer_start, timer_end 
        FROM iShark_Partners_News
        ORDER BY title 
    ";
