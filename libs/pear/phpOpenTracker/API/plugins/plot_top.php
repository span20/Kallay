...
class phpOpenTracker_API_plot_top extends phpOpenTracker_API_Plugin {
  ...
  */
  function run($parameters) {
	...
	$title = $locale->get('admin_stat', 'field_top').' '.$parameters['limit'].' ';

    switch ($parameters['what']) {
      case 'document': {
        $title .= $locale->get('admin_stat', 'field_totalpages');
      }
      break;
      ...
      case 'host': {
        $title .= $locale->get('admin_stat', 'field_host');
      }
      break;

      case 'operating_system': {
        $title .= $locale->get('admin_stat', 'field_opsyss');
      }
      break;

      case 'referer': {
        $title .= $locale->get('admin_stat', 'field_referer');
      }
      break;

      case 'user_agent': {
        $title .= $locale->get('admin_stat', 'field_browsers');
      }
      break;
    }

    $title .= " (".$locale->get('admin_stat', 'field_total').": $total)";
